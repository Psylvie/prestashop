...
<script type="text/javascript">
    ...
    var id_customer = 0;
    var admin_order_tab_link = "{$link->getAdminLink('AdminAccountManagerOrders')|addslashes|escape:'html':'UTF-8'|replace:'&amp;':'&'}";
    ...
    var admin_cart_link = '{$link->getAdminLink('AdminAccountManagerCarts')|addslashes|escape:'html':'UTF-8'|replace:'&amp;':'&'}';
    ...
    $(document).ready(function() {
        ...
        $("#voucher").autocomplete('{$link->getAdminLink('AdminCartRules')|addslashes}', {
                    ...
                    parse: function(data) {
                        if (!data.found)
                            $('#vouchers_err').html('{l s='No voucher was found' mod='accountmanager'}').show();
                        else
                            $('#vouchers_err').hide();
                        var mytab = new Array();
        ...
        $('input:radio[name="free_shipping"]').on('change',function() {
            ...
            $.ajax({
                type:"POST",
                url: "{$link->getAdminLink('AdminAccountManagerCarts')|addslashes|escape:'html':'UTF-8'|replace:'&amp;':'&'}",
                async: true,
                dataType: "json",
                data : {
                    ajax: "1",
        ...
        $('#order_message').live('change', function(e) {
            ...
            $.ajax({
                type:"POST",
                url: "{$link->getAdminLink('AdminCarts')|addslashes|escape:'html':'UTF-8'|replace:'&amp;':'&'}",
                async: true,
                dataType: "json",
                data : {
                    ajax: "1",
        ...
        $('#customer_part').on('click','button.setup-customer',function(e){
            ...
            $(this).removeClass('setup-customer').addClass('change-customer').html('<i class="icon-refresh"></i>&nbsp;{l s='Change' mod='accountmanager'}').blur();
    ...
    function searchCustomers()
    {
        $.ajax({
            type:"POST",
            url : "{$link->getAdminLink('AdminAccountManagerCustomers')|escape:'html':'UTF-8'|replace:'&amp;':'&'}",
            async: true,
            dataType: "json",
            data : {
                ajax: "1",
                tab: "AdminAccountManagerCustomers",
            ...
            success : function(res)
            {
                if(res.found)
                {
                    ...
                    $.each(res.customers, function() {
                        ...
                        html += '<a href="{$link->getAdminLink('AdminAccountManagerCustomers')|escape:'html':'UTF-8'|replace:'&amp;':'&'}&id_customer='+this.id_customer+'&viewcustomer&liteDisplaying=1" class="btn btn-default fancybox"><i class="icon-search"></i> {l s='Details' mod='accountmanager'}</a>';
                        html += '<button type="button" data-customer="'+this.id_customer+'" class="setup-customer btn btn-default pull-right"><i class="icon-arrow-right"></i> {l s='Choose' mod='accountmanager'}</button>';
                        html += '</div>';
                        html += '</div>';
                        html += '</div>';
                    });
                    ...
                    html = '<div class="alert alert-warning">{l s='No customers found' mod='accountmanager'}</div>';
                $('#customers').html(html);
                resetBind();
            }
        });
            ...
            success : function(res)
            {
                if(res.found)
                {
                    ...
                    $.each(res.carts, function() {
                        ...
                        html_carts += '<a title="{l s='View this cart' mod='accountmanager'}" class="fancybox btn btn-default" href="index.php?tab=AdminAccountManagerCarts&id_cart='+this.id_cart+'&viewcart&token={getAdminToken tab='AdminAccountManagerCarts'}&liteDisplaying=1#"><i class="icon-search"></i>&nbsp;{l s='Details' mod='accountmanager'}</a>';
                        html_carts += '&nbsp;<a href="#" title="{l s='Use this cart' mod='accountmanager'}" class="use_cart btn btn-default" rel="'+this.id_cart+'"><i class="icon-arrow-right"></i>&nbsp;{l s='Use' mod='accountmanager'}</a>';
                        html_carts += '</td>';
                        html_carts += '</tr>';
                    });
                    ...
                    $.each(res.orders, function() {
                        ...
                        html_orders += '<a href="{$link->getAdminLink('AdminAccountManagerOrders')|escape:'html':'UTF-8'|replace:'&amp;':'&'}&id_order='+this.id_order+'&vieworder&liteDisplaying=1#" title="{l s='View this order' mod='accountmanager'}" class="fancybox btn btn-default"><i class="icon-search"></i>&nbsp;{l s='Details' mod='accountmanager'}</a>';
                        html_orders += '&nbsp;<a href="#" "title="{l s='Duplicate this order' mod='accountmanager'}" class="duplicate_order btn btn-default" rel="'+this.id_order+'"><i class="icon-arrow-right"></i>&nbsp;{l s='Use' mod='accountmanager'}</a>';
                        html_orders += '</td>';
                        html_orders += '</tr>';
                    });
    ...
    function updateDeliveryOptionList(delivery_option_list)
    {
        ...
        {
            $('#carrier_form').hide();
            $('#carriers_err').show().html('{l s='No carrier can be applied to this order' mod='accountmanager'}');
    ...
    function searchProducts()
    {
        ...
        $.ajax({
            type:"POST",
            url: "{$link->getAdminLink('AdminAccountManagerOrders')|addslashes|escape:'html':'UTF-8'|replace:'&amp;':'&'}",
            async: true,
            dataType: "json",
            data : {
                ajax: "1",
                token: "{$token|escape:'html':'UTF-8'}",
                tab: "AdminAccountManagerOrders",
            ...
            success : function(res)
            {
                ...
                if(res.found)
                {
                    ...
                    $('#products_found').show();
                    products_found += '<label class="control-label col-lg-3">{l s='Product' mod='accountmanager'}</label><div class="col-lg-6"><select id="id_product" onclick="display_product_attributes();display_product_customizations();"></div>';
                    attributes_html += '<label class="control-label col-lg-3">{l s='Combination' mod='accountmanager'}</label><div class="col-lg-6">';
                    $.each(res.products, function() {
                        ...
                        {
                            customization_html += '<div class="bootstrap"><div class="panel"><div class="panel-heading">{l s='Customization' mod='accountmanager'}</div><form id="customization_'+id_product+'" class="id_customization" method="post" enctype="multipart/form-data" action="'+admin_cart_link+'" style="display:none;">';
                ...
                {
                    $('#products_found').hide();
                    $('#products_err').html('{l s='No products found' mod='accountmanager'}');
    ...
    function updateCartProducts(products, gifts, id_address_delivery)
    {
        ...
        $.each(gifts, function() {
            ...
            cart_content += '<td>{l s='Gift' mod='accountmanager'}</td><td>'+this.cart_quantity+'</td><td>{l s='Gift' mod='accountmanager'}</td></tr>';
        });
        ...
        if (typeof(vouchers) == 'object')
            $.each(vouchers, function(){
                if (parseFloat(this.value_real) === 0 && parseInt(this.free_shipping) === 1)
                    var value = '{l s='Free shipping' mod='accountmanager'}';
                else
                    var value = this.value_real;

                vouchers_html += '<tr><td>'+this.name+'</td><td>'+this.description+'</td><td>'+value+'</td><td class="text-right"><a href="#" class="btn btn-default delete_discount" rel="'+this.id_discount+'"><i class="icon-remove text-danger"></i>&nbsp;{l s='Delete' mod='accountmanager'}</a></td></tr>';
            });
    ...
    function sendMailToCustomer()
    {
        $.ajax({
            ...
            data : {
                ajax: "1",
                token: "{getAdminToken tab='AdminAccountManagerOrders'}",
                tab: "AdminAccountManagerOrders",
    ...
    function updateAddressesList(addresses, id_address_delivery, id_address_invoice)
    {
        ...
        $.each(addresses, function() {
            if (this.id_address == id_address_invoice)
            {
                ...
                invoice_address_edit_link = "{$link->getAdminLink('AdminAccountManagerAddresses')|escape:'html':'UTF-8'|replace:'&amp;':'&'}&id_address="+this.id_address+"&updateaddress&realedit=1&liteDisplaying=1&submitFormAjax=1#";
            }

            if(this.id_address == id_address_delivery)
            {
                address_delivery_detail = this.formated_address;
                delivery_address_edit_link = "{$link->getAdminLink('AdminAccountManagerAddresses')|escape:'html':'UTF-8'|replace:'&amp;':'&'}&id_address="+this.id_address+"&updateaddress&realedit=1&liteDisplaying=1&submitFormAjax=1#";
            }
        ...
        {
            $('#addresses_err').show().html('{l s='You must add at least one address to process the order.' mod='accountmanager'}');
...
<div class="leadin">{block name="leadin"}{/block}</div>
    <div class="panel form-horizontal" id="customer_part">
        <div class="panel-heading">
            <i class="icon-user"></i>
            {l s='Customer' mod='accountmanager'}
        </div>
        <div id="search-customer-form-group" class="form-group">
            <label class="control-label col-lg-3">
                <span title="" data-toggle="tooltip" class="label-tooltip" data-original-title="{l s='Search for an existing customer by typing the first letters of his/her name.' mod='accountmanager'}">
                    {l s='Search for a customer' mod='accountmanager'}
                </span>
            </label>
            <div class="col-lg-9">
                <div class="row">
                    ...
                    <div class="col-lg-6">
                        <span class="form-control-static">{l s='Or' mod='accountmanager'}&nbsp;</span>
                        <a class="fancybox_customer btn btn-default" href="{$link->getAdminLink('AdminAccountManagerCustomers')|escape:'html':'UTF-8'}&amp;addcustomer&amp;liteDisplaying=1&amp;submitFormAjax=1#">
                            <i class="icon-plus-sign-alt"></i>
                            {l s='Add new customer' mod='accountmanager'}
                        </a>
                    </div>
                </div>
            </div>
            ...
            <ul id="old_carts_orders_navtab" class="nav nav-tabs">
                <li class="active">
                    <a href="#nonOrderedCarts" data-toggle="tab">
                        ...
                        {l s='Carts' mod='accountmanager'}
                    </a>
                </li>
                <li>
                    <a href="#lastOrders" data-toggle="tab">
                        <i class="icon-credit-card"></i>
                        {l s='Orders' mod='accountmanager'}
                    </a>
                </li>
            </ul>
            <div id="old_carts_orders" class="tab-content panel collapse in">
                <div id="nonOrderedCarts" class="tab-pane active">
                    <table class="table">
                        <thead>
                            <tr>
                                <th><span class="title_box">{l s='ID' mod='accountmanager'}</span></th>
                                <th><span class="title_box">{l s='Date' mod='accountmanager'}</span></th>
                                <th><span class="title_box">{l s='Total' mod='accountmanager'}</span></th>
                                <th></th>
                            </tr>
                        </thead>
                        <tbody>
                    ...
                    <table class="table">
                        <thead>
                            <tr>
                                <th><span class="title_box">{l s='ID' mod='accountmanager'}</span></th>
                                <th><span class="title_box">{l s='Date' mod='accountmanager'}</span></th>
                                <th><span class="title_box">{l s='Products' mod='accountmanager'}</span></th>
                                <th><span class="title_box">{l s='Total paid' mod='accountmanager'}</span></th>
                                <th><span class="title_box">{l s='Payment' mod='accountmanager'}</span></th>
                                <th><span class="title_box">{l s='Status' mod='accountmanager'}</span></th>
                                <th></th>
                            </tr>
                        </thead>
                        <tbody>
...
<form class="form-horizontal" action="{$link->getAdminLink('AdminAccountManagerOrders')|escape:'html':'UTF-8'}&amp;submitAdd{$table|escape:'html':'UTF-8'}=1" method="post" autocomplete="off">
    <div class="panel" id="products_part" style="display:none;">
        <div class="panel-heading">
            <i class="icon-shopping-cart"></i>
            {l s='Cart' mod='accountmanager'}
        </div>
        <div class="form-group">
            <label class="control-label col-lg-3">
                <span title="" data-toggle="tooltip" class="label-tooltip" data-original-title="{l s='Search for an existing product by typing the first letters of its name.' mod='accountmanager'}">
                    {l s='Search for a product' mod='accountmanager'}
                </span>
            </label>
            <div class="col-lg-9">
        ...
        <div id="products_found">
            ...
            <div class="form-group">
                <label class="control-label col-lg-3" for="qty">{l s='Quantity' mod='accountmanager'}</label>
                <div class="col-lg-9">
                    <input type="text" name="qty" id="qty" class="form-control fixed-width-sm" value="1" />
                    <p class="help-block">{l s='In stock' mod='accountmanager'} <span id="qty_in_stock"></span></p>
                </div>
            </div>
            ...
            <div class="form-group">
                <div class="col-lg-9 col-lg-offset-3">
                    ...
                    {l s='Add to cart' mod='accountmanager'}
                </div>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-lg-12">
                <table class="table" id="customer_cart">
                    <thead>
                        <tr>
                            <th><span class="title_box">{l s='Product' mod='accountmanager'}</span></th>
                            <th><span class="title_box">{l s='Description' mod='accountmanager'}</span></th>
                            <th><span class="title_box">{l s='Reference' mod='accountmanager'}</span></th>
                            <th><span class="title_box">{l s='Unit price' mod='accountmanager'}</span></th>
                            <th><span class="title_box">{l s='Quantity' mod='accountmanager'}</span></th>
                            <th><span class="title_box">{l s='Price' mod='accountmanager'}</span></th>
                        </tr>
                    </thead>
                    <tbody>
                    </tbody>
        ...
        <div class="form-group">
            <div class="col-lg-9 col-lg-offset-3">
                <div class="alert alert-warning">{l s='The prices are without taxes.' mod='accountmanager'}</div>
            </div>
        </div>

        <div class="form-group">
            <label class="control-label col-lg-3" for="id_currency">
                {l s='Currency' mod='accountmanager'}
            </label>
            <script type="text/javascript">
            ...
            <div class="col-lg-9">
                <select id="id_currency" name="id_currency">
                    {foreach from=$currencies item='currency'}
                        <option rel="{$currency.iso_code|escape:'htmlall':'UTF-8'}" value="{$currency.id_currency|escape:'htmlall':'UTF-8'}">{$currency.name|escape:'htmlall':'UTF-8'}</option>
                    {/foreach}
                </select>
            </div>
        </div>
        <div class="form-group">
            <label class="control-label col-lg-3" for="id_lang">
                {l s='Language' mod='accountmanager'}
            </label>
            <div class="col-lg-9">
    ...
    <div class="panel" id="vouchers_part" style="display:none;">
        <div class="panel-heading">
            ...
            {l s='Vouchers' mod='accountmanager'}
        </div>
        <div class="form-group">
            <label class="control-label col-lg-3">
                {l s='Search for a voucher' mod='accountmanager'}
            </label>
            <div class="col-lg-9">
                <div class="row">
                    <div class="col-lg-6">
                    ...
                    <div class="col-lg-6">
                        ...
                        <a class="fancybox btn btn-default" href="{$link->getAdminLink('AdminCartRules')|escape:'html':'UTF-8'}&amp;addcart_rule&amp;liteDisplaying=1&amp;submitFormAjax=1#">
                            <i class="icon-plus-sign-alt"></i>
                            {l s='Add new voucher' mod='accountmanager'}
                        </a>
                    </div>
                </div>
            </div>
        ...
        <div class="row">
            <table class="table" id="voucher_list">
                <thead>
                    <tr>
                        <th><span class="title_box">{l s='Name' mod='accountmanager'}</span></th>
                        <th><span class="title_box">{l s='Description' mod='accountmanager'}</span></th>
                        <th><span class="title_box">{l s='Value' mod='accountmanager'}</span></th>
                        <th></th>
                    </tr>
                </thead>
                <tbody>
    ...
    <div class="panel" id="address_part" style="display:none;">
        <div class="panel-heading">
            ...
            {l s='Addresses' mod='accountmanager'}
        </div>
        ...
        <div class="row">
            <div id="address_delivery" class="col-lg-6">
                <h4>
                    ...
                    {l s='Delivery' mod='accountmanager'}
                </h4>
                <div class="row-margin-bottom">
                    <select id="id_address_delivery" name="id_address_delivery"></select>
                </div>
                <div class="well">
                    <a href="" id="edit_delivery_address" class="btn btn-default pull-right fancybox"><i class="icon-pencil"></i> {l s='Edit' mod='accountmanager'}</a>
            ...
            <div id="address_invoice" class="col-lg-6">
                ...
                <div class="well">
                    <a href="" id="edit_invoice_address" class="btn btn-default pull-right fancybox"><i class="icon-pencil"></i> {l s='Edit' mod='accountmanager'}</a>
        ...
        <div class="row">
            <div class="col-lg-12">
                <a class="fancybox btn btn-default" id="new_address" href="{$link->getAdminLink('AdminAccountManagerAddresses')|escape:'html':'UTF-8'}&amp;addaddress&amp;id_customer=42&amp;liteDisplaying=1&amp;submitFormAjax=1#">
                    <i class="icon-plus-sign-alt"></i>
                    {l s='Add a new address' mod='accountmanager'}
                </a>
            </div>
        </div>
    </div>
    <div class="panel" id="carriers_part" style="display:none;">
        <div class="panel-heading">
            <i class="icon-truck"></i>
            {l s='Shipping' mod='accountmanager'}
        </div>
        <div id="carriers_err" style="display:none;" class="alert alert-warning"></div>
        <div id="carrier_form">
            <div class="form-group">
                <label class="control-label col-lg-3">
                    {l s='Delivery option' mod='accountmanager'}
                </label>
                <div class="col-lg-9">
            ...
            <div class="form-group">
                <label class="control-label col-lg-3" for="shipping_price">
                    {l s='Shipping price (Tax incl.)' mod='accountmanager'}
                </label>
                <div class="col-lg-9">
            ...
            <div class="form-group">
                <div class="checkbox col-lg-9 col-offset-3">
                    <label for="carrier_recycled_package">
                        ...
                        {l s='Recycled package' mod='accountmanager'}
                    </label>
                </div>
            </div>
            {/if}
            ...
            <div class="form-group">
                <div class="checkbox col-lg-9 col-offset-3">
                    <label for="order_gift">
                        ...
                        {l s='Gift' mod='accountmanager'}
                    </label>
                </div>
            </div>
            <div class="form-group">
                <label class="control-label col-lg-3" for="gift_message">{l s='Gift message' mod='accountmanager'}</label>
                <div class="col-lg-9">
    ...
    <div class="panel" id="summary_part" style="display:none;">
        <div class="panel-heading">
            ...
            {l s='Summary' mod='accountmanager'}
        </div>
            ...
            <div class="row">
                <div class="col-lg-2">
                    <div class="data-focus">
                        <span>{l s='Total products' mod='accountmanager'}</span><br/>
                        <span id="total_products" class="size_l text-success"></span>
                    </div>
                </div>
                <div class="col-lg-2">
                    <div class="data-focus">
                        <span>{l s='Total vouchers (Tax excl.)' mod='accountmanager'}</span><br/>
                        <span id="total_vouchers" class="size_l text-danger"></span>
                    </div>
                </div>
                <div class="col-lg-2">
                    <div class="data-focus">
                        <span>{l s='Total shipping (Tax excl.)' mod='accountmanager'}</span><br/>
                        <span id="total_shipping" class="size_l"></span>
                    </div>
                </div>
                <div class="col-lg-2">
                    <div class="data-focus">
                        <span>{l s='Total taxes' mod='accountmanager'}</span><br/>
                        <span id="total_taxes" class="size_l"></span>
                    </div>
                </div>
                <div class="col-lg-2">
                    <div class="data-focus">
                        <span>{l s='Total (Tax excl.)' mod='accountmanager'}</span><br/>
                        <span id="total_without_taxes" class="size_l"></span>
                    </div>
                </div>
                <div class="col-lg-2">
                    <div class="data-focus data-focus-primary">
                        <span>{l s='Total (Tax incl.)' mod='accountmanager'}</span><br/>
        ...
        <div class="row">
            <div class="order_message_right col-lg-12">
                <div class="form-group">
                    <label class="control-label col-lg-3" for="order_message">{l s='Order message' mod='accountmanager'}</label>
                    <div class="col-lg-6">
                ...
                <div class="form-group">
                    ...
                    <div class="col-lg-9 col-lg-offset-3">
                        <a href="javascript:void(0);" id="send_email_to_customer" class="btn btn-default">
                            <i class="icon-credit-card"></i>
                            {l s='Send an email to the customer with the link to process the payment.' mod='accountmanager'}
                        </a>
                        <a id="go_order_process" href="" class="btn btn-link _blank">
                            {l s='Go on payment page to process the payment.' mod='accountmanager'}
                ...
                <div class="form-group">
                    <label class="control-label col-lg-3">{l s='Payment' mod='accountmanager'}</label>
                    <div class="col-lg-9">
                        <select name="payment_module_name" id="payment_module_name">
                            ...
                            {/foreach}
                            {else}
                                <option value="boorder">{l s='Back office order' mod='accountmanager'}</option>
                            {/if}
                        </select>
                    </div>
                </div>
                <div class="form-group">
                    <label class="control-label col-lg-3">{l s='Order status' mod='accountmanager'}</label>
                    <div class="col-lg-9">
                        <select name="id_order_state" id="id_order_state">
                            {foreach from=$order_states item='order_state'}
                                <option value="{$order_state.id_order_state|escape:'htmlall':'UTF-8'}" {if isset($smarty.post.id_order_state) && $order_state.id_order_state == $smarty.post.id_order_state}selected="selected"{/if}>{$order_state.name|escape:'htmlall':'UTF-8'}</option>
                            {/foreach}
                        </select>
                    </div>
                </div>
                <div class="form-group">
                    <div class="col-lg-9 col-lg-offset-3">
                        <button type="submit" name="submitAddOrder" class="btn btn-default" />
                            ...
                            {l s='Create the order' mod='accountmanager'}
                        </button>
                    </div>
                </div>
            </div>
